<%@ page language="java" contentType="text/html; charset=UTF-8"%>
<link rel="stylesheet" type="text/css" href="${ctx}/resources/extjs/resources/css/ext-all.css" />
<link id="theme" rel="stylesheet" type="text/css" href="${ctx}/resources/extjs/resources/css/xtheme-blue.css" />
<script type="text/javascript" src="${ctx}/resources/extjs/adapter/ext/ext-base.js"></script>
<script type="text/javascript" src="${ctx}/resources/extjs/ext-all-debug.js"></script>
<script type="text/javascript" src="${ctx}/resources/extjs/ext-lang-zh_CN.js"></script>
<script type="text/javascript" src="${ctx}/resources/extjs/ux/ExtMD5.js"></script>
<script type="text/javascript" src="${ctx}/resources/js/share.js"></script>
<script type="text/javascript">
	ctx = "${ctx}";
	Ext.BLANK_IMAGE_URL = '${ctx}/resources/extjs/resources/images/default/s.gif';
</script>