...
Ext.ns("Ext.Phoenix.role"); // 自定义一个命名空间
role = Ext.Phoenix.role; // 定义命名空间的别名
role = {
	all : ctx + '/role',// 加载所有
									...
									callback : function(json) {
										...
										if (json.length > 0) {
											for (var i = 0; i < json.length; i++) {
												...
												var treeNode = role.treePanel.getNodeById("_phoenix_" + moduleId);
												if(treeNode)
	...
	if (treeNode.hasChildNodes()) {
		for (i = 0; i < treeNode.childNodes.length; i++) {
			if (treeNode.childNodes[i].getUI().checkbox) {
				if (treeNode.childNodes[i].getUI().checkbox.checked) {
					// 去除前缀
					role.childNodes += treeNode.childNodes[i].id.replace("_phoenix_", "") + ',';
				}
			}
		}
		for (i = 0; i < treeNode.childNodes.length; i++) {
