...
Ext.ns("Ext.Phoenix.user"); // 自定义一个命名空间
user = Ext.Phoenix.user; // 定义命名空间的别名
user = {
    all: ctx + '/user',
    // 所有用户
    save: ctx + "/user/save",
...
user.pageSize = parseInt(user.pageSizeCombo.getValue()); /** 基本信息-数据源 */
...
user.tbar = [user.addAction, '-', user.editAction, '-', user.deleteAction, '-', user.resetPwdAction, '-', user.searchField]; /** 底部工具条 */
...
user.addWindow = new Ext.Window({
    ...
    buttons: [{
        ...
        handler: function () {
            ...
            form.reset();
            if (id != '') form.findField("userId").setValue(id);
            if (account != '') form.findField("account").setValue(account);
        }
    }]
});
