Ext.ns("Ext.Phoenix.field"); // 自定义一个命名空间
field = Ext.Phoenix.field; // 定义命名空间的别名
field = {
    all: ctx + '/field/all',
    // 加载所有
    save: ctx + "/field/save",
...
field.pageSize = parseInt(field.pageSizeCombo.getValue()); /** 基本信息-数据源 */
field.store = new Ext.data.Store({
    ...
    }, ['fieldId', 'fieldName', 'field', 'valueField', 'displayField', 'enabled', 'sort']),
    listeners: {
...
field.tips = '&nbsp;<font color="red"><b>提示:维护字段后必须执行内存同步</b></font>'; /** 顶部工具栏 */
field.tbar = [field.addAction, '-', field.editAction, '-', field.deleteAction, '-', field.searchField, '-', field.synchroAction, field.tips]; /** 底部工具条 */
...
field.addWindow = new Ext.Window({
    ...
    buttons: [{
        ...
        handler: function () {
            ...
            form.reset();
            if (id != '') form.findField("fieldId").setValue(id);
        }
    }]
});
