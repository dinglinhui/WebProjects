Ext.ns("Ext.Phoenix.modules"); // 自定义一个命名空间
module = Ext.Phoenix.modules; // 定义命名空间的别名
module = {
    all: ctx + '/module/all',
    // 加载所有
    save: ctx + "/module/save",
    //保存
    del: ctx + "/module/del/",
    //删除
...
module.pageSize = parseInt(module.pageSizeCombo.getValue()); /** 基本信息-数据源 */
module.store = new Ext.data.Store({
    ...
    }, ['moduleId', 'moduleName', 'moduleUrl', 'parentId', 'leaf', 'expanded', 'displayIndex', 'isDisplay', 'iconCss', 'information']),
    listeners: {
...
module.tbar = [module.addAction, '-', module.editAction, '-', module.deleteAction, '-', module.searchField]; /** 底部工具条 */
...
module.formPanel = new Ext.form.FormPanel({
    ...
    module.leafCombo, module.expandedCombo, module.isDisplayradiogroup,
    {
        fieldLabel: 'CSS样式',
        maxLength: 128,
        allowBlank: false,
...
module.addWindow = new Ext.Window({
    ...
    buttons: [{
        ...
        handler: function () {
            ...
            form.reset();
            if (id != '') form.findField("moduleId").setValue(id);
        }
    }]
});
