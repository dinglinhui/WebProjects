package org.paswork.framework.support.dao.hibernate;

import org.hibernate.*;
import org.hibernate.criterion.DetachedCriteria;
import org.hibernate.type.Type;
import org.paswork.framework.support.dao.util.ConditionQuery;
import org.paswork.framework.support.dao.util.OrderBy;
import org.paswork.framework.util.Assert;
...
public abstract class BaseHibernateDao<M extends java.io.Serializable, PK extends java.io.Serializable> implements IBaseDao<M, PK> {
    ...
    public List<M> listAll(int pn, int pageSize) {
        return listit(HQL_LIST_ALL, pn, pageSize);
    }
    ...
    public List<M> pre(PK pk, int pn, int pageSize) {
        ...
        //倒序，重排
        List<M> result = listit(HQL_OPTIMIZE_PRE_LIST_ALL, 1, pageSize, pk);
    ...
    public List<M> next(PK pk, int pn, int pageSize) {
        ...
        return listit(HQL_OPTIMIZE_NEXT_LIST_ALL, 1, pageSize, pk);
    }
    ...
    @SuppressWarnings("unchecked")
    protected <T> List<T> listit(final String hql, final int pn, final int pageSize, final Object... paramlist) {
        Query query = getSession().createQuery(hql);
        setParameters(query, paramlist);
        if (pn > -1 && pageSize > -1) {
    ...
    protected <T> List<T> list(final String sql, final Object... paramlist) {
        return listit(sql, -1, -1, paramlist);
    }
